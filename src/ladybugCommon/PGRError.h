...
// #define PGR_FILE_NAME    "$RCSfile: PGRError.h $"
//
//=============================================================================
...
{ \
   ...
   char _szLastPGRErrorMessage_[1024]; \
   time_t ltime; \
   time( &ltime ); \
   sscanf( PGR_FILE_NAME+1,"RCSfile: %s $", _szFileName_ ); \
   if ( ERROR_NUMBER == 0 ) \
   { \
      sprintf(_szLastPGRErrorMessage_,"%s (line %d)\nBuild Time: %s %s\nCurrent Time: %s\n\nError: %s", \
      _szFileName_, __LINE__,__DATE__,__TIME__, ctime( &ltime ), ERROR_MESSAGE ); \
   }\
   else \
   { \
      sprintf(_szLastPGRErrorMessage_,"%s (line %d)\nBuild Time: %s %s\nCurrent Time: %s\n\nError(0x%x): %s", \
      _szFileName_, __LINE__,__DATE__,__TIME__, ctime( &ltime), ERROR_NUMBER, ERROR_MESSAGE ); \
   } \
   ...
   { \
      printf( "%s", _szLastPGRErrorMessage_ ); \
   }\
}
...
{ \
   ...
   if ( ERROR_NUMBER == 0 ) \
   { \
      sprintf( _szLastPGRErrorMessage_,"%s (line %d) Error: %s\n", \
      _szFileName_, __LINE__, ERROR_MESSAGE ); \
   }\
   else \
   { \
      sprintf( _szLastPGRErrorMessage_,"%s (line %d) Error(0x%x): %s\n", \
      _szFileName_, __LINE__, ERROR_NUMBER, ERROR_MESSAGE ); \
   } \
   if ( MESSAGE_LEVEL >= ERROR_LEVEL ) \
   { \
      fprintf( stderr, "%s", _szLastPGRErrorMessage_ ); \
   }\
}
